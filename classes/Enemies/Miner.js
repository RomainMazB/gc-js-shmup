class Miner extends EnemyBase {
    constructor(pGameObjectManager, pCollisionsManager, pTransform, pAssetAtlas) {
        super(pGameObjectManager, pTransform);

        let fighterComp = this.addComponent(new Fighter(10, 10))
        fighterComp.setWeapon(new MultiParallelBullets(pGameObjectManager, pCollisionsManager, pAssetAtlas, Projectile, ENEMY_PROJECTILES, this, 1))
        fighterComp.weapon.isFiring = true
        fighterComp.weapon.direction = Math.PI /2

        let rb = this.getComponent(RigidBody)

        let spriteAnimator = new Animator('./assets/images/enemies/miner.png', 88, 96, -44, -55)
