class Orc extends EnemyBase {
    constructor(pGameObjectManager, pCollisionsManager, pTransform, pAssetAtlas) {
        ...
        let fighterComp = this.addComponent(new Fighter(3, 3))
        fighterComp.setWeapon(new SingleBullet(pGameObjectManager, pCollisionsManager, pAssetAtlas, Projectile, ENEMY_PROJECTILES, this, 1))
        fighterComp.weapon.isFiring = true
        fighterComp.weapon.direction = Math.PI /2

        let rb = this.getComponent(RigidBody)

        let spriteAnimator = new Animator('./assets/images/enemies/orc.png', 88, 96, -44, -55)
